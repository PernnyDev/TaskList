...
public class AuthService {
	...
	public String authenticate(String email, String pwd) {
		...
		if(user.isPresent() && user.get().getPassword().equals(pwd)) {
			...
			String token = String.format("%s|%s|%d|%d", user.get().getEmail(),user.get().getNickname(),user.get().getId(), Instant.now().toEpochMilli()			);
	...
	public User toUser(String token) {
		String[] split = toUserArray(token);
		return new User(Integer.valueOf(split[2]),split[0],null,split[1]);
